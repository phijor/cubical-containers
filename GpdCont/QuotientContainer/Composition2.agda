...
module GpdCont.QuotientContainer.Composition2 where
...
module _ {ℓ}
  ...
  (Q : T → hSetSub)
  (G : S → Group ℓ)
  (σ : (s : S) → Action (G s) ⟨ Representative (P s) ⟩ˢ)
  (H : T → Group ℓ)
  (τ : (t : T) → Action (H t) ⟨ Representative (Q t) ⟩ˢ)
  where
  open hasSigma has-Σ
  ...
  private
    P' : (s : S) → Type _
    P' s = ⟨ Representative (P s) ⟩

    Q' : (t : T) → Type _
    Q' t = ⟨ Representative (Q t) ⟩
