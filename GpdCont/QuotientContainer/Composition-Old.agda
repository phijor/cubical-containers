...
module _ {ℓ}
  ...
  rwd : {s : S} {f f' : ⟨ P s ⟩ → T} (g₀ : ⟨ G s ⟩) (rel : f ≡ f' ∘ (σ _ ⁺ g₀))
    → ⟨ R* s f ⟩ ≃ ⟨ R* s f' ⟩
  rwd g₀ rel = Σ-cong-equiv (σ.action g₀) λ p → substEquiv (⟨_⟩ ∘ Q) (rel ≡$ p)
  ...
  ∣R∣ = uncurry λ s → SQ.rec isSetCard (∣_∣₂ ∘ R* s) (well-defined s) where
    module _ (s : S) where
      ...
      well-defined f f' = ∃-rec (ST.isSetSetTrunc _ _) λ g f≡f'σ →
        cong ∣_∣₂ $ uaSub (R* s f) (R* s f') $ rwd g f≡f'σ
  ...
  -- XXX: Is this an embedding of groups?
  module _ {s} (f : ⟨ P s ⟩ → T) where
    private
      R' = (R* s f)

      Rep : hSetSub → Type _
      Rep = λ - → ⟨ Representative - ⟩

      is-set-Rep : isSet (Rep R')
      is-set-Rep = isSub→isSet $ str (Representative R')

      is-set-R : isSet ⟨ R' ⟩
      is-set-R = isSub→isSet $ str R'

    posRepMap : ⟨ R' ⟩ ≃ ⟨ R' ⟩ → Rep R' ≃ Rep R'
    posRepMap = repMap' R' R'

    posRepHom : GroupHom (𝔖 ⟨ R* s f ⟩ˢ) (𝔖 ⟨ R (s , [ f ]) ⟩ˢ)
    posRepHom .fst = posRepMap
    posRepHom .snd = makeIsGroupHom $ repMapComp R' R' R'

    -- XXX: This is probably not true!
    -- isEquivPosRepMap : isEquiv posRepMap
    -- isEquivPosRepMap = isEquivRepMap R' R'
